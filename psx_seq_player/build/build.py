...
ninja.rule("psyq_c_preprocess_44", "$psyq_c_preprocessor_44_exe -undef -D__GNUC__=2 -D__OPTIMIZE__ " + includes + " -lang-c -Dmips -D__mips__ -D__mips -Dpsx -D__psx__ -D__psx -D_PSYQ -D__EXTENSIONS__ -D_MIPSEL -D__CHAR_UNSIGNED__ -D_LANGUAGE_C -D__cplusplus -D_LANGUAGE_C_PLUS_PLUS $in $out", "Preprocess $in -> $out")
ninja.newline()

ninja.rule("convert_c_encoding", f"{sys.executable} convjp.py $in $out", "Convert $in from UTF-8 to EUC-JP")
ninja.newline()

# generate header deps, adds edges to the build graph for the next build -M option will write header deps
ninja.rule("psyq_c_preprocess_44_headers", "$psyq_c_preprocessor_44_exe -M -undef -D_LANGUAGE_C_PLUS_PLUS -D__cplusplus -D__GNUC__=2 -D__OPTIMIZE__ " + includes + " -lang-c -Dmips -D__mips__ -D__mips -Dpsx -D__psx__ -D__psx -D_PSYQ -D__EXTENSIONS__ -D_MIPSEL -D__CHAR_UNSIGNED__  $in $out", "Preprocess for includes $in -> $out")
ninja.newline()
...
ninja.rule("unix2dos", f"unix2dos -n $in $out", "Preprocess $in -> $out")
ninja.newline()
...
ninja.rule("psylink", f"$psyq_psylink_exe /l {psqy_lib} /c /n 4000 /q /gp .sdata /m @\"../{args.obj_directory}/linker_command_file_dos$suffix.txt\",../{args.obj_directory}/_mgsi$suffix.cpe,../{args.obj_directory}/asm$suffix.sym,../{args.obj_directory}/asm$suffix.map", "Link $out")



#ninja.rule("psylink", f"$psyq_psylink_exe -Xo$80010000 /l {psqy_lib} ../vs_vt.obj ../vs_vtc.obj ../main.obj,{psqy_lib}/LIBSN.LIB,{psqy_lib}/LIBDS.LIB,{psqy_lib}/LIBAPI.LIB,{psqy_lib}/LIBGPU.LIB,{psqy_lib}/LIBCD.LIB,{psqy_lib}/LIBGS.LIB,{psqy_lib}/LIBC.LIB,{psqy_lib}/LIBETC.LIB,{psqy_lib}/LIBGTE.LIB", "Link $out")
#ninja.newline()

ninja.rule("create_dummy_file_overlays", f"{sys.executable} create_dummy_file.py $src_dir/../{args.obj_directory}/$overlay_bin $src_dir/../{args.obj_directory}/$overlay_bss_bin", "Create dummy files $overlay_bin, $overlay_bss_bin")
ninja.newline()
...
def gen_build_target(targetName):
    ...
    # cFiles = ["../SEQPLAYER.CPP"]
    cFiles = ['../vs_vt.c', '../vs_vtc.c', '../seqplayer.c', '../main.c', '../lib_snd.c', '../lib_spu.c']

    # cFiles = get_files_recursive("../", ".c")
    print(cFiles)
    # exit(0)
    ...
    linkerCommandFileDos = f"../{args.obj_directory}/linker_command_file_dos.txt"
    ninja.build(linkerCommandFileDos, "unix2dos", linkerCommandFile)
    ninja.newline()

    # run the linker to generate the cpe
    cpeFile = f"../{args.obj_directory}/_mgsi.cpe"
    ninja.build(cpeFile, "psylink", implicit=linkerDeps + [linkerCommandFileDos])
    ninja.newline()
